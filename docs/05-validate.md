...
>
> **Stand:** Die qualitative Usability-Evaluation wurde durchgeführt, ausgewertet und in konkrete Produkt-Verbesserungen überführt. Die Unterrichtsevaluation und die privaten Nachtests werden getrennt ausgewiesen, damit Umfang, Evidenz und Limitationen nachvollziehbar bleiben.
...
Historisch wurden im Unterricht zunächst zwei Mitstudierende detailliert protokolliert; die konsolidierte Unterrichtsauswertung wurde anschliessend um eine dritte Unterrichtsrückmeldung und zwei private Nachtests ergänzt. Dadurch umfasst die Dokumentation insgesamt fünf anonymisierte Testpersonen, wobei die unterschiedliche Protokolltiefe transparent gemacht wird.
...
| Datum Nachtests (P4–P5) | **24. Mai 2026** |
| Hinweis zur Version | Die Tests fanden auf der öffentlich erreichbaren Netlify-Version statt. Die Live-URL blieb während der Iteration gleich; nach der Evaluation wurde der Prototyp basierend auf den Erkenntnissen weiter verbessert (siehe Abschnitt 11). Der finale Abgabestand ist zusätzlich mit Screenshots unter [`docs/screenshots/`](screenshots/README.md) dokumentiert. |
| Evidenz / Archivierung | Es gibt kein separates Deployment-Archiv der Zwischenversion vom 20. Mai 2026. Die Evaluation wird deshalb über Testdatum, Aufgaben, anonymisierte Stichprobe, Feedback Grid, Issue-Liste, Schweregrade und dokumentierte Verbesserungen belegt. Die Unterschiede zwischen getesteter Version und finalem Stand sind in Abschnitt 11 explizit als umgesetzte Verbesserungen nachvollziehbar. |
...
- Vergleichbares moderiertes Vorgehen am **24. Mai 2026**.
- Fokus ergänzend auf den erweiterten Workflows (Argumentgewichtung, Voting-Journal, Kompass), weil diese nach der ersten Unterrichtsrunde stärker ausgebaut wurden.
...
|---|---|---|
| **A** | «Du möchtest dich über eine aktuelle Abstimmung informieren — z. B. die Nachhaltigkeitsinitiative. Öffne die App und finde heraus, wie die wichtigsten Parteien dazu Stellung beziehen.» | Navigation, Briefing, Parteipositionen |
...
| P4 (Nachtest, privat) | A–E (Hauptworkflow + Kompass) | Neutrale Aufbereitung, geführter Ablauf und Kompass-Ranking waren verständlich | Keine neuen Probleme mit hoher Priorität beobachtet | — | — | Der Nachtest bestätigte, dass der erweiterte Workflow auch bei niedrigerem politischem Vorwissen bedienbar blieb; einzelne Detailbeobachtungen wurden qualitativ, nicht quantitativ protokolliert | 0–1 |
| P5 (Nachtest, privat) | A–F (Hauptworkflow + Profil + Kompass) | Strukturierte Informationsdarstellung, geführter Aufbau und Wiederfinden gespeicherter Inhalte funktionierten im Testablauf | Keine neuen Probleme mit hoher Priorität beobachtet | — | — | Der Nachtest bestätigte den Nutzen des Voting-Journals als Reflexionsraum; die Beobachtung wurde bewusst als Plausibilisierung der Iteration gewertet, nicht als statistischer Nachweis | 0–1 |

> Die Zeilen für P4 und P5 fassen den dokumentierten Eindruck der privaten Nachtests zusammen. Die Nachtests dienen als qualitative Plausibilisierung der erweiterten Workflows; es wurden keine exakten Zitate, Zeiten, Erfolgsquoten oder Klickzahlen ergänzt, weil diese nicht systematisch erhoben wurden.
...
| U5 | Briefing-Seite / Footer | Quellenhinweis wurde übersehen | Quellen waren nur unscheinbar im Footer | Konsolidiert | 2 | Quellen pro Argument inline, separate Quellen-Seite | **Umgesetzt** (Quellen-Sektion auf Detailseite, eigene Quellen- und Medienberichte-Seite, final mit Screenshot dokumentiert) |
...
- Nicht alle Beobachtungen wurden während des Unterrichtstests gleich formal protokolliert. Die vorhandene Dokumentation aus zwei detailliert protokollierten Mitstudierenden, die konsolidierte Unterrichtsrückmeldung und die zwei privaten Nachtests bilden gemeinsam die Grundlage dieser Auswertung.
- Die getestete Zwischenversion wurde nicht als separates Deployment konserviert. Stattdessen dokumentiert diese Datei den Verlauf von getesteter Version zu finalem Prototyp über konkrete Issues, Schweregrade und umgesetzte Verbesserungen; der finale Stand ist in [`screenshots/`](screenshots/README.md) visuell belegt.
