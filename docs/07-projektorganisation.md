...
| Admin-Auth ist bewusst prototypisch (ein Passwort-Cookie, keine Rollen) | [`hooks.server.ts`](../src/hooks.server.ts) + [`admin/login/+page.server.ts`](../src/routes/admin/login/+page.server.ts) | Für einen Prototyp ausreichend, für ein Produktivsystem zu schlicht | Rollen/Rechte, Hashing und Session-Store ergänzen; `ADMIN_PASSWORD` bleibt zwingend als Env-Variable gesetzt |
